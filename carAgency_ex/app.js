...
// console.log(getCarsToBuyByModelAndAgencyId(ourCarMarket.sellers, 3, '26_IPfHU1'));
...
//  console.log(ourCarMarket.customers[1].cars);
//! ---------------- Setters -----------------
...
const setNewCarToAgency = (agencyObject, carObject) => {
  // console.log(agencyObject.cars[carObject.brand]);
  // check if brand exists before push or create array for the new brand
  agencyObject.cars[carObject.brand].push(carObject);
  return carObject;
};
const agency = ourCarMarket.sellers[2];
// console.log(setNewCarToAgency(agency, { brand: "Bmw", name: "X12" }));
...
const deleteCarFromAgency = (agencyObject, carNumber) => {
  ...
  for (let brand in agencyObject.cars) {
    const carToDelete = agencyObject.cars[brand].find((car) => {
        if (car.carNumber === carNumber) {
          deletedCar = car;
        }
      });
      const arrayToDeleteFrom = agencyObject.cars[brand].filter((car) => {
...
// const agency2 = ourCarMarket.sellers[0];
// console.log(deleteCarFromAgency(agency2, "4Ixb0"));
...
//  console.log(decOrIncCashOfAgency(agency, 20)); 
...
const setCarToCustomer = (customerObj, carObject) => {
  // check if customer.cars is an array -> push
  console.log(ourCarMarket.customers[0].cars);
  if (ourCarMarket.customers[0].cars) {
    ourCarMarket.customers[0].cars.push(newCarObj)
  } else {
    ourCarMarket.customers[0].cars = [carObject];
  }
  // if not - create customer.cars = [carObject]
  // return customer.cars
  return ourCarMarket.customers[0].cars;
};
(setCarToCustomer(ourCarMarket.customers.cars, newCarObj)); 
