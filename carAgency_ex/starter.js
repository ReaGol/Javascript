...
const getAgencyByName = (agenciesArr, name) => {
  return agenciesArr.filter((item) => item.agencyName.includes(name));
};
...
const getAgencyById = function (agenciesArr, id) {
  return agenciesArr.find((item) => item.agencyId === id);
};
...
const getCarsToBuy = (agenciesArr) => {
  ...
  agenciesArr.forEach((agency) => {
    // OPTION 1
    for (let car in agency.cars) {
      carsArr.push(agency.cars[car]);
    }
    // OPTION 2
    // for (const [key, value] of Object.entries(agency.cars)) {
    //   if (value.length > 0) {
    //     carsArr.push(value)
    //   }
    // }
    //***option 3 */ for (let carsArr of Object.values(cars))
  });
  // return carsArr;
  return carsArr.flat();
};
...
getCarsToBuyByModel(ourCarMarket.sellers, "Spider");
